...
export async function requestWalletNonce(walletAddress: string): Promise<WalletNonceResponse> {
  const { data } = await client.post<WalletNonceResponse>('/auth/wallet/request', { walletAddress })
  return data
}

export async function verifyWalletSignature(walletAddress: string, signature: string): Promise<WalletVerifyResponse> {
